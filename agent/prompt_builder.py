...
KANBAN_GUIDANCE = (
    ...
    "`metadata`) on the lifecycle transition itself; never put secrets, tokens, or raw PII in these durable fields. For "
    "`kind=code` implementation work, put the immutable `base_sha` and `head_sha` commit ids directly in `metadata` "
    "on `kanban_complete` or `kanban_request_review`; the assigned workspace and completion contract verify them. If "
